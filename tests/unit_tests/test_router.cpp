...
TEST(RouterTests, DynamicRouteMultipleParamsMatch) {
  // GIVEN:
  Router::instance().addRoute("GET", "/post/{year}/{slug}",
...
TEST(RouterTests, DynamicRouteOrderTest) {
  ...
  Router::instance().addRoute(
      "GET", "/user/settings", [](const HttpRequest& req) {
...
TEST(RouterTests, WildcardRouteLongestPrefixWins) {
  ...
  // Create a file in imgDir that should be served when the longer prefix
  // matches
  ...
  Router::instance().addStaticDirectoryRoute("/static/",
                                             baseDir.string() + "/");
  Router::instance().addStaticDirectoryRoute("/static/images/",
                                             imgDir.string() + "/");
...
TEST(RouterTests, DynamicBeatsWildcardWhenBothMatch) {
  // GIVEN:
  Router::instance().addRoute(
      "GET", "/file/{name}", [](const HttpRequest& req) {
        return Responses::ok(req, "dynamic", "text/plain");
      });
